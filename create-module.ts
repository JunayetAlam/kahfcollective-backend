...
moduleNames.forEach((moduleName) => {
  ...
  const pascalName = toPascalCase(lowerCaseName);
  const baseDir = path.join(__dirname, 'src', 'app', 'modules', moduleName); // Keep original casing for folder

  const files = [
    { name: `${lowerCaseName}.controller.ts`, type: 'controller' },
    { name: `${lowerCaseName}.interface.ts`, type: 'interface' },
    { name: `${lowerCaseName}.route.ts`, type: 'route' },
    { name: `${lowerCaseName}.service.ts`, type: 'service' },
    { name: `${lowerCaseName}.validation.ts`, type: 'validation' },
  ];
